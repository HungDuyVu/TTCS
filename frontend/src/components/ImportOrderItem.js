...
const ImportOrderItem = ({ data, fetchData }) => {
        ...
        const fetchSupplierById = async () => {
            try {
                ...
                if (supplierData.success) {
                    setSupplierName(supplierData.data.name);
                } else {
                    console.error("Failed to fetch supplier:", supplierData.message);
                }
            } catch (error) {
    ...
    return (
        <div className='bg-gray-100 p-6 m-4 shadow-md rounded-lg'>
            ...
            <div className='mb-2 text-gray-700'>
                <strong className='text-lg'>Nhà cung cấp:</strong> <span>{supplierName}</span>
            </div>
            <div className='mb-2 text-gray-700'>
